...
function validar_diagnosticos_hc(ind_obligatorio) {
	return validar_diagnosticos_hc_ojos(ind_obligatorio, true);
}

function validar_diagnosticos_hc_ojos(ind_obligatorio, ind_ojos) {
	var ind_error = 0;
...
function copiar_dignosticos_ant(id_hc) {
	var params = "opcion=3&id_hc=" + id_hc;
	
	llamarAjax("../funciones/Class_Diagnosticos_ajax.php", params, "d_diagnosticos_ant", "continuar_copiar_dignosticos_ant();");
}

function continuar_copiar_dignosticos_ant() {
	var cant_ant = parseInt($("#hdd_cant_ant_dg").val(), 10);
	if (cant_ant > 0) {
		$("#lista_tabla").val(cant_ant);
		
		for (var i = 1; i <= 10; i++) {
			if (i <= cant_ant) {
				$("#tabla_diag_" + i).css("display", "block");
				
				$("#ciex_diagnostico_" + i).val($("#hdd_cod_ciex_ant_dg_" + i).val());
				$("#hdd_ciex_diagnostico_" + i).val($("#hdd_cod_ciex_ant_dg_" + i).val());
				$("#texto_diagnostico_" + i).html($("#hdd_nom_ciex_ant_dg_" + i).val());
				$("#valor_ojos_" + i).val($("#hdd_ojo_ant_dg_" + i).val());
			} else {
				if (i > 4) {
					$("#tabla_diag_" + i).css("display", "none");
				}
				
				$("#ciex_diagnostico_" + i).val("");
				$("#hdd_ciex_diagnostico_" + i).val("");
				$("#texto_diagnostico_" + i).html("");
				$("#valor_ojos_" + i).val("");
			}
		}
	} else {
		alert("No se encontraron diagn\xf3sticos anteriores.");
	}
}
