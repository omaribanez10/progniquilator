...
function agregar_numero_factura(num_factura) {
	...
	//Usuarios
	var cont_aux = 0;
	while ($("#txt_num_factura_us_" + cont_aux).length > 0) {
		$("#txt_num_factura_us_" + cont_aux).val(num_factura);
...
function guardar_ac() {
	...
	if (ind_ac == 1) {
		var params = "opcion=7&id_rips=" + $("#hdd_id_rips").val() +
		...
		for (var i = 0; i < cant_registros_ac; i++) {
			if ($("#tr_reg_ac_" + i).css("display") != "none") {
				params += "&id_admision_ac_" + cont_aux + "=" + $("#hdd_id_admision_ac_" + i).val() +
		...
		llamarAjax("generador_rips_ajax.php", params, "d_guardar_ac", "finalizar_guardar_rips(\"ac\");");
	}
}
...
function guardar_ap() {
	...
	if (ind_ap == 1) {
		var params = "opcion=8&id_rips=" + $("#hdd_id_rips").val() +
					 "&id_convenio=" + $("#hdd_id_convenio").val() +
					 "&id_plan=" + $("#hdd_id_plan").val() +
					 "&fecha_ini=" + $("#hdd_fecha_ini").val() +
					 "&fecha_fin=" + $("#hdd_fecha_fin").val() +
		...
		for (var i = 0; i < cant_registros_ap; i++) {
			if ($("#tr_reg_ap_" + i).css("display") != "none") {
				params += "&id_admision_ap_" + cont_aux + "=" + $("#hdd_id_admision_ap_" + i).val() +
		...
		llamarAjax("generador_rips_ajax.php", params, "d_guardar_ap", "finalizar_guardar_rips(\"ap\");");
	}
}
...
function guardar_am() {
	...
	if (ind_am == 1) {
		var params = "opcion=9&id_rips=" + $("#hdd_id_rips").val() +
					 "&id_convenio=" + $("#hdd_id_convenio").val() +
					 "&id_plan=" + $("#hdd_id_plan").val() +
					 "&fecha_ini=" + $("#hdd_fecha_ini").val() +
					 "&fecha_fin=" + $("#hdd_fecha_fin").val() +
		...
		for (var i = 0; i < cant_registros_am; i++) {
			if ($("#tr_reg_am_" + i).css("display") != "none") {
				params += "&id_admision_am_" + cont_aux + "=" + $("#hdd_id_admision_am_" + i).val() +
		...
		llamarAjax("generador_rips_ajax.php", params, "d_guardar_am", "finalizar_guardar_rips(\"am\");");
	}
}
...
function guardar_at() {
	...
	if (ind_at == 1) {
		var params = "opcion=10&id_rips=" + $("#hdd_id_rips").val() +
					 "&id_convenio=" + $("#hdd_id_convenio").val() +
					 "&id_plan=" + $("#hdd_id_plan").val() +
					 "&fecha_ini=" + $("#hdd_fecha_ini").val() +
					 "&fecha_fin=" + $("#hdd_fecha_fin").val() +
		...
		for (var i = 0; i < cant_registros_at; i++) {
			if ($("#tr_reg_at_" + i).css("display") != "none") {
				params += "&id_admision_at_" + cont_aux + "=" + $("#hdd_id_admision_at_" + i).val() +
		...
		llamarAjax("generador_rips_ajax.php", params, "d_guardar_at", "finalizar_guardar_rips(\"at\");");
	}
}
...
function guardar_us() {
	...
	if (ind_us == 1) {
		...
		for (var i = 0; i < cant_registros_us; i++) {
			if ($("#tr_reg_us_" + i).css("display") != "none") {
		...
		llamarAjax("generador_rips_ajax.php", params, "d_guardar_us", "finalizar_guardar_rips(\"us\");");
	}
}
...
function guardar_af() {
	...
	if (ind_af == 1) {
		...
		llamarAjax("generador_rips_ajax.php", params, "d_guardar_af", "finalizar_guardar_rips(\"af\");");
	}
}
...
function guardar_ad() {
	...
	if (ind_ad == 1) {
		...
		llamarAjax("generador_rips_ajax.php", params, "d_guardar_ad", "finalizar_guardar_rips(\"ad\");");
	}
}

function finalizar_guardar_rips(sigla) {
		...
		for (var i = 0; i < cant_registros_ac; i++) {
			if ($("#tr_reg_ac_" + i).css("display") != "none") {
				...
				if (num_factura_aux in mapa_valores) {
					var valor_aux = mapa_valores[num_factura_aux] + parseFloat("0" + $("#txt_valor_consulta_ac_" + i).val());
					mapa_valores[num_factura_aux] = valor_aux;
				} else {
					mapa_valores[num_factura_aux] = parseFloat("0" + $("#txt_valor_consulta_ac_" + i).val());
	...
	for (var num_factura in mapa_valores) {
		...
		for (var i = 0; i < cant_registros_af; i++) {
			if ($("#txt_num_factura_af_" + i).val() == num_factura) {
				...
				$("#txt_valor_neto_af_" + i).val(mapa_valores[num_factura]);
		...
		if (!bol_hallado) {
			if ($("#txt_num_factura_af_" + cant_registros_af).length > 0) {
				...
				$("#txt_valor_neto_af_" + cant_registros_af).val(mapa_valores[num_factura]);
...
function generar_rips() {
	if (validar_generar_rips()) {
		...
		if ($("#hdd_ac").val()) {
			setTimeout(function() { guardar_ac(); }, tiempo_espera);
			tiempo_espera += 100;
		}
		if ($("#hdd_ap").val()) {
			setTimeout(function() { guardar_ap(); }, tiempo_espera);
			tiempo_espera += 100;
		}
		if ($("#hdd_am").val()) {
			setTimeout(function() { guardar_am(); }, tiempo_espera);
			tiempo_espera += 100;
		}
		if ($("#hdd_at").val()) {
			setTimeout(function() { guardar_at(); }, tiempo_espera);
			tiempo_espera += 100;
		}
		if ($("#hdd_us").val()) {
			setTimeout(function() { guardar_us(); }, tiempo_espera);
			tiempo_espera += 100;
		}
		if ($("#hdd_af").val()) {
			setTimeout(function() { guardar_af(); }, tiempo_espera);
			tiempo_espera += 100;
		}
		if ($("#hdd_ad").val()) {
			setTimeout(function() { guardar_ad(); }, tiempo_espera);
			tiempo_espera += 100;
		}
